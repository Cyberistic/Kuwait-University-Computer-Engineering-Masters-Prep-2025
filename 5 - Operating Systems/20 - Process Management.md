...
1.  new processes need to go to the [New] section, it handles many processes at a time, the processes come from the *disk*. The processes are being **created** here.

2. After the process goes through the [New queue] it goes to the [Ready queue] were the instructions are being **executed**. The [Ready queue] is in the *main memory*.

3. From the [Ready queue] only one process gets out at a time. It has the option either to be selected by the *scheduler dispatch* to go to the [Running] state or it stays in the current queue. 
...
3. **Turnaround time** – the total time the process needed from the response time till completion *(end time - response time)* with respect to waiting time.
